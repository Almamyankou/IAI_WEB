...
<div class="container">
    ...
    <table class="table">
        ...
        <tbody>
            <?php
            // Assurez-vous que les données des candidats sont correctement récupérées
            $sqlCandidats = "SELECT nom, prenom, nationalite, serie_bac, sexe FROM candidats";
            $resultCandidats = $conn->query($sqlCandidats);

            if ($resultCandidats->num_rows > 0) {
                while ($rowCandidat = $resultCandidats->fetch_assoc()) {
                    echo "<tr>";
                    echo "<td>" . $rowCandidat['nom'] . "</td>";
                    echo "<td>" . $rowCandidat['prenom'] . "</td>";
                    echo "<td>" . $rowCandidat['nationalite'] . "</td>";
                    echo "<td>" . $rowCandidat['serie_bac'] . "</td>";
                    echo "<td>" . $rowCandidat['sexe'] . "</td>";
                    echo "</tr>";
                }
            } else {
                echo "<tr><td colspan='5'>Aucun candidat inscrit.</td></tr>";
            }
            ?>
        </tbody>
    </table>
...
<script>
    ...
    var candidatsData = {
        labels: ['Nationalité', 'Série Bac', 'Sexe'],
        datasets: [{
            label: 'Nombre de candidats',
            data: [
                <?php echo $totalCandidatesByNationalitySum; ?>,
                <?php echo $totalCandidatesBySeriesSum; ?>,
                <?php echo $totalCandidatesBySexSum; ?>
            ],
            backgroundColor: [
                'rgba(255, 99, 132, 0.2)', // Rouge pour la nationalité
                'rgba(54, 162, 235, 0.2)',  // Bleu pour la série Bac
                'rgba(255, 206, 86, 0.2)'   // Jaune pour le sexe
            ],
            borderColor: [
                'rgba(255, 99, 132, 1)',
                'rgba(54, 162, 235, 1)',
